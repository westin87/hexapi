...
from hexarpi.utils.deriver import PDRegulator


class HexacopterRegulator:
    def __init__(self, position_alpha=0.1, direction_alpha=0.6):
    ...
    def update(self, position, direction, target_position):
        ...
        roll, pitch = self._direction_to_pitch_and_rool(relative_direction_to_travel)
    ...
    def _calculate_angle_to_target(self, target_direction):
        return np.arccos(Point2D.dot(self._direction_estimate, target_direction))
    ...
    @staticmethod
    def _direction_to_pitch_and_rool(relative_direction_to_travel):
