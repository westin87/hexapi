...
class Orientation():
    ...
    def __init__(self):
        ...
        if self._read_byte(LSM303D, 0x0F) == LSM303D_WHO_AM_I:
            print "OR: LSM303D detected successfully."
        else:
            print "OR: No LSM303D detected"

        if self._read_byte(L3GD20H, 0x0F) == L3GD20H_WHO_AM_I:
    ...
    def _read_byte(self, address, register):
        return self.__i2c_bus.read_byte_data(address, register)

    def _configure_LSM303D(self):
        print "OR: Configuring LSM303D"
        self._write_byte(LSM303D, LSM303D_CTRL_1,
                         0b01010111)  # enable accelerometer, 50 hz sampling
        self._write_byte(LSM303D, LSM303D_CTRL_2,
                         0b00000000)  # set +/- 2g full scale
        self._write_byte(LSM303D, LSM303D_CTRL_5,
                         0b01100100)  # high resolution mode, thermometer off, 6.25hz ODR
        self._write_byte(LSM303D, LSM303D_CTRL_6,
                         0b00100000)  # set +/- 4 gauss full scale
        self._write_byte(LSM303D, LSM303D_CTRL_7,
                         0b00000000)  # get magnetometer out of low power mode

    def _configure_L3GD20H(self):
        print "OR: Configuring L3GD20H"
        self._write_byte(L3GD20H, L3GD20H_CTRL_1,
                         0b10001111)  # enable gyroscope, set 50 hz data rate
        self._write_byte(L3GD20H, L3GD20H_CTRL_2,
                         0b00000000)  # set normal hp filter, set 4 hz cut off
        self._write_byte(L3GD20H, L3GD20H_CTRL_4,
                         0b00000000)  # continues updates
        self._write_byte(L3GD20H, L3GD20H_CTRL_5,
                         0b00000000)  # no FIFO, no HP filter

    def _combine_two_comp(self, msb, lsb):
        value = (msb << 8) | lsb
    ...
    def get_acceleration(self):
        acceleration_x = self._combine_two_comp(
            self._read_byte(LSM303D, LSM303D_ACC_X_MSB),
            self._read_byte(LSM303D, LSM303D_ACC_X_LSB))
        acceleration_y = self._combine_two_comp(
            self._read_byte(LSM303D, LSM303D_ACC_Y_MSB),
            self._read_byte(LSM303D, LSM303D_ACC_Y_LSB))
        acceleration_z = self._combine_two_comp(
            self._read_byte(LSM303D, LSM303D_ACC_Z_MSB),
            self._read_byte(LSM303D, LSM303D_ACC_Z_LSB))

        return acceleration_x, acceleration_y, acceleration_z

    def get_magnetic_field(self):
        magnetic_x = self._combine_two_comp(
            self._read_byte(LSM303D, LSM303D_MAG_X_MSB),
            self._read_byte(LSM303D, LSM303D_MAG_X_LSB))
        magnetic_y = self._combine_two_comp(
            self._read_byte(LSM303D, LSM303D_MAG_Y_MSB),
            self._read_byte(LSM303D, LSM303D_MAG_Y_LSB))
        magnetic_z = self._combine_two_comp(
            self._read_byte(LSM303D, LSM303D_MAG_Z_MSB),
            self._read_byte(LSM303D, LSM303D_MAG_Z_LSB))

        return magnetic_x, magnetic_y, magnetic_z

    def get_angular_rate(self):
        angular_rate_x = self._combine_two_comp(
            self._read_byte(L3GD20H, L3GD20H_COM_X_MSB),
            self._read_byte(L3GD20H, L3GD20H_COM_X_LSB))
        angular_rate_y = self._combine_two_comp(
            self._read_byte(L3GD20H, L3GD20H_COM_Y_MSB),
            self._read_byte(L3GD20H, L3GD20H_COM_Y_LSB))
        angular_rate_z = self._combine_two_comp(
            self._read_byte(L3GD20H, L3GD20H_COM_Z_MSB),
            self._read_byte(L3GD20H, L3GD20H_COM_Z_LSB))
